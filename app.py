...
def invite():
    ...
    if recommended_holiday is None:
        ...
        confirmation = intent_confirmation_layer(response_assistant, conversation)
        if "No" in confirmation:
            conversation.append({"role": "assistant", "content": response_assistant})
            conversation_bot.append({'bot': response_assistant})
        else:
            response = dictionary_present(response_assistant, user_input)
            conversation_bot.append({'bot': "Thank you for providing all the information. Kindly wait, while I fetch the holidays:"})

            recommended_holiday = compare_holiday_with_user(response, user_input)
            if not recommended_holiday or len(json.loads(recommended_holiday)) == 0:
                conversation_bot.append({'bot': "Sorry, we do not have holidays matching your requirements. Connecting you to a human expert. Please end this conversation."})
            else:
