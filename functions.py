...
def intent_confirmation_layer(response_assistant, messages):
    delimiter = "####"
    prompt = f"""
    ...
    confirmation = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=messages,  # Ensure messages parameter is included
        temperature=0
    )
...
def dictionary_present(response, messages):
    delimiter = "####"
    ...
    if isinstance(messages, str):
        messages = [{"role": "user", "content": messages}]  # Convert to a list if messages is a string        
    messages.append({"role": "system", "content": prompt})
...
def extract_dictionary_from_string(string):
    ...
    # Initialize dictionary to ensure it is always defined
    dictionary = {}
...
def product_map_layer(Destination, Package, Origin, Duration, messages):
    delimiter = "#####"
    holiday_spec = {
  ...
  Extract all the features above according to the following rules: 
  {delimiter}
  ...
  """
    ...
    # Append the system message with the instruction and the user message with the holiday package info
    messages.append({"role": "system", "content": prompt})
    user_message = f"Holiday Package: {Destination}, {Package}, {Origin}, {Duration}"
...
def compare_holiday_with_user(user_req_string, messages):
    # Load the dataset
   # Load the dataset
    holiday_df = pd.read_csv('triptravel2data.csv')

        # Preprocess the 'Transportation cost' column
    holiday_df['Transportation cost'] = (
        holiday_df['Transportation cost']
        .astype(str)                                   # Convert to string
        .str.replace(r'[^\d]', '', regex=True)         # Remove non-numeric characters (e.g., $)
        .replace('', '0')                             # Replace empty strings with '0'
        .astype(int)                                  # Convert to integer
    )

    # Debug: Print the cleaned column to verify
    print(holiday_df['Transportation cost'])
    ...
    holiday_df['holiday_feature'] = holiday_df.apply(
        lambda x: product_map_layer(x['Destination'], x['Package Type'], x['Start City'], x['Itinerary'], messages),
        axis=1
    )
    # Extract user requirements into a dictionary
    user_requirements = extract_dictionary_from_string(user_req_string)

    # Retrieve the budget from the user requirements, removing non-numeric characters
    budget = int(user_requirements.get('budget', '0').replace(',', '').replace('$', '').split()[0])

    # Filter the holiday dataframe to include only rows where the 'Transportation cost' is <= budget
    filtered_holiday = holiday_df[holiday_df['Transportation cost'] <= budget].copy()

    # Safely delete the 'budget' key if it exists
    if 'budget' in user_requirements:
        del user_requirements['budget']

    # Extract holiday packages into a dictionary so they can be compared with user requirements
    filtered_holiday['holiday_feature'] = filtered_holiday['holiday_feature'].apply(
        lambda x: extract_dictionary_from_string(x)
    )

    # Comparison of user requirements with holiday packages
    mask = filtered_holiday['holiday_feature'].apply(
        lambda x: all(x.get(key) == user_requirements[key] for key in user_requirements)
    )
